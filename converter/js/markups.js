...
const getCurrencyItemAction = (isBase) => {
    ...
    return `
        <button data-action="${actionName}" class="currency-${actionName} currency-button">${actionName}</button>
    `
}
...
export const renderCurrencyItem = ({ code, base_code: baseCode, rate = 1 }) => {
    ...
    return `
        <div class="currency-item ${isBase ? 
            "currency-current" : ""}" data-item="${code}">
