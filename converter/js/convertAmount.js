...
const {
    success, formResults,
    rateConversion, rateLast,
    toSelect, fromSelect,
    resultFrom, resultTo } =
    variables

export const handleChangeCode = ({ target: { value, name } }) => {
    state.pair = { ...state.pair, [name]: value,}
}

export const handleInputAmount = ({ target: { value, name } }) => {
